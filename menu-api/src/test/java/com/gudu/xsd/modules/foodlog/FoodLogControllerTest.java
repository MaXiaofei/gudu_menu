...
class FoodLogControllerTest {
    ...
    void GET_月视图_统计卡和时间轴() throws Exception {
        ...
        given(svc.month(any(), eq(2026), eq(7), eq(null), eq(null), eq(null)))
                .willReturn(new FoodLogService.MonthVO(summary, List.of(meal)));
        ...
        verify(svc).month(any(), eq(2026), eq(7), eq(null), eq(null), eq(null));
    }
