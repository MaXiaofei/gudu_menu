 ...
 *
 * <p>主料进 items（计入进度），调料（purchaseCategoryId=调味料品类）折叠到 condiments（不计进度）。
 */
@Service
@RequiredArgsConstructor
public class MenuPrepService {

    /** 调味料品类 id（shopping 已用，盐/油/豉油 无需备料，折叠到 condiments 组）。 */
    ...
    /** GET /menu/{id}/prep：聚合全量用料 + 备料状态 + 共用高亮 + 调料折叠 + 进度。 */
    public MenuPrepVO getPrep(Long menuId) {
        ...
        return new MenuPrepVO(items, condiments, readyCount, items.size());
    }
