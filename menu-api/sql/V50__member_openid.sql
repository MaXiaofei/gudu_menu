...
SET @s1 := IF(@c1=0,
  'ALTER TABLE member ADD COLUMN openid VARCHAR(64) NULL COMMENT ''微信 openid（小程序静默登录用）''',
  'SELECT 1');
SET @k1 := (SELECT COUNT(*) FROM information_schema.STATISTICS
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'member' AND INDEX_NAME = 'uk_openid');
SET @sk := IF(@k1=0,'ALTER TABLE member ADD UNIQUE KEY uk_openid (openid)','SELECT 1');
PREPARE stmt FROM @s1;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
PREPARE stmt FROM @sk;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
