...
--   脏菜系（家常菜/清真/日料/韩餐/西餐——非国家认可菜系体系）同名并入 tag，
--   菜谱关联信息不丢（dish_dict 改挂 tag）。
--
...
-- 1) 确保目标 tag 存在（已存在则 IGNORE 跳过，重复执行安全）
...
-- 验证：
--   SELECT id, name FROM sys_dict WHERE dict_group='cuisine' ORDER BY sort;  -- 应只剩 10 个
--   SELECT s.name, COUNT(*) FROM dish_dict d JOIN sys_dict s ON s.id=d.dict_id
--     WHERE d.rel_type='tag' AND s.name IN ('家常菜','清真','日料','韩餐','西餐') GROUP BY s.name;
--   SELECT COUNT(*) orphan FROM dish_dict d LEFT JOIN sys_dict s ON s.id=d.dict_id WHERE s.id IS NULL;  -- 应为 0
